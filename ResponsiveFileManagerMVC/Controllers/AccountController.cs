...
namespace ResponsiveFileManagerMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email.ToUpper(), Email = model.Email + "@tailyn.com.tw" };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    var staffs = new Staff { StaffId = model.Email.ToUpper(), StaffName = model.Name };
                    db.Staff.Add(staffs);
                    db.SaveChanges();
